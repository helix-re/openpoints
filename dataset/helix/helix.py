...
print("je rentre dans helix dataset")
...
class Helix(Dataset): # class S3DIS(Dataset):
    ...
    """
    def __init__(self,
                 data_root: str = 'data/helix/helix',
                 test_area: int = 4000,
        ...
        filename = os.path.join(
            processed_root, f'helix_{split}_area{test_area}_{voxel_size:.3f}_{str(voxel_max)}.pkl')
        if presample and not os.path.exists(filename):
            np.random.seed(0)
            self.data = []
            for item in tqdm(self.data_list, desc=f'Loading HELIX {split} split on Test Area {test_area}'):
                data_path = os.path.join(raw_root, item + '.npy')
                cdata = np.load(data_path,allow_pickle=True).astype(np.float32) # cdata = np.load(data_path).astype(np.float32)
    ...
    def __getitem__(self, idx):
        ...
        if self.presample:
            ...
            cdata = np.load(data_path,allow_pickle=True).astype(np.float32)
            cdata[:, :3] -= np.min(cdata[:, :3], 0)
